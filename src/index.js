...
function createServer() {
    ...
    server.registerTool(
        'scrape',
        {
            ...
            inputSchema: z.object({
                ...
                removeSelector: z.string().optional().describe('CSS selector to remove from page before scraping'),
                onlyMainContent: z.boolean().optional().describe('Only extract main content, excluding navigation and footers'),
                includeLinks: z.boolean().optional().describe('Include extracted links in response')
            })
        },
        async ({ url, waitForSelector, timeout, removeSelector, onlyMainContent, includeLinks }) => {
            try {
                ...
                if (onlyMainContent !== undefined) requestBody.onlyMainContent = onlyMainContent;
    ...
    server.registerTool(
        'create-session',
        {
            ...
            inputSchema: z.object({
                sessionId: z.string().uuid().optional().describe('Custom session ID (UUID). If not provided, Steel generates one.'),
                ...
                recordVideo: z.boolean().optional().describe('Record video of the session'),
    ...
    server.registerTool(
        ...
        async ({ sessionId }) => {
            try {
                ...
                return { 
                    content: [{ type: 'text', text: `Session ${sessionId} released successfully.` }] 
                };
            } catch (error) {
                return { 
                    content: [{ type: 'text', text: `Error releasing session ${sessionId}: ${error.message}` }], 
                    isError: true 
                };
            }
        }
    ...
    server.registerTool(
        ...
        async ({ sessionId }) => {
            try {
                ...
                return { 
                    content: [{ type: 'text', text: `Error getting session ${sessionId}: ${error.message}` }], 
                    isError: true 
                };
            }
        }
    ...
    server.registerTool(
        'navigate',
        {
            ...
            inputSchema: z.object({
                ...
                takeScreenshot: z.boolean().optional().describe('Take a screenshot after navigation')
            })
        },
        async ({ sessionId, url, waitForSelector, timeout, takeScreenshot }) => {
            try {
                ...
                if (waitForSelector) requestBody.waitForSelector = waitForSelector;
                if (timeout) requestBody.timeout = timeout;
                if (takeScreenshot !== undefined) requestBody.takeScreenshot = takeScreenshot;

                const result = await steelRequest(`/v1/sessions/${sessionId}/navigate`, {
                    method: 'POST',
                    body: JSON.stringify(requestBody)
                });
                ...
                return { 
                    content: [{ type: 'text', text: `Error navigating in session ${sessionId}: ${error.message}` }], 
                    isError: true 
                };
            }
        }
    ...
    server.registerTool(
        'execute-script',
        {
            ...
            inputSchema: z.object({
                ...
                awaitPromise: z.boolean().optional().describe('Wait for the script to return a promise')
            })
        },
        async ({ sessionId, script, awaitPromise }) => {
            try {
                ...
                return { 
                    content: [{ type: 'text', text: `Error executing script in session ${sessionId}: ${error.message}` }], 
                    isError: true 
                };
            }
        }
    ...
    server.registerTool(
        ...
        async ({ sessionId, fullPage, selector }) => {
            try {
                ...
                const result = await steelRequest(`/v1/sessions/${sessionId}/screenshot`, {
                    method: 'POST',
                    body: JSON.stringify(requestBody)
                });
                ...
                return { 
                    content: [{ type: 'text', text: `Error taking screenshot in session ${sessionId}: ${error.message}` }], 
                    isError: true 
                };
            }
        }
    ...
    server.registerTool(
        'get-content',
        {
            ...
            inputSchema: z.object({
                ...
                format: z.enum(['html', 'markdown', 'text']).optional().describe('Content format to return')
            })
        },
        async ({ sessionId, format }) => {
            try {
                ...
                return { 
                    content: [{ type: 'text', text: `Error getting content from session ${sessionId}: ${error.message}` }], 
                    isError: true 
                };
            }
        }
